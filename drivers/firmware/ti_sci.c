 ...
 */
static int ti_sci_cmd_ring_allocate(const struct ti_sci_handle *handle,
				    ...
				    u16 type)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_RING_ALLOCATE,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
		dev_err(info->dev, "RM_RA:Message alloc failed(%d)\n", ret);
		return ret;
	}
	...
	ret = ti_sci_do_xfer(info, xfer);
	if (ret) {
		dev_err(info->dev, "RM_RA:Mbox send fail %d\n", ret);
		goto fail;
	}

	resp = (struct ti_sci_msg_ring_allocate_resp *)xfer->tx_message.buf;
...
fail:
	dev_dbg(info->dev, "RM_RA:allocate ring %u ret:%d\n", *index, ret);
	return ret;
}
				...
				u8 s_host, u32 nav_id, u32 index)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_RING_FREE,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
...
fail:
	dev_dbg(info->dev, "RM_RA:free ring %u ret:%d\n", index, ret);
	return ret;
}
				 ...
				 u32 nav_id, u32 index)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_RING_RESET,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
...
fail:
	dev_dbg(info->dev, "RM_RA:reset ring %u ret:%d\n", index, ret);
	return ret;
}
 ...
 */
static int ti_sci_cmd_ring_reconfig(const struct ti_sci_handle *handle,
				    ...
				    u8 *order_id)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_RING_RECONFIG,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
		dev_err(info->dev, "RM_RA:Message reconfig failed(%d)\n", ret);
		return ret;
	}
	req.nav_id = nav_id;
	req.index = index;
	...
	resp = (struct ti_sci_msg_ring_reconfig_resp *)xfer->tx_message.buf;
...
fail:
	dev_dbg(info->dev, "RM_RA:reconfig ring %u ret:%d\n", index, ret);
	return ret;
}
			...
			u32 *index)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_UDMAP_TX_ALLOC,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
	...
	resp = (struct ti_sci_msg_udmap_tx_ch_alloc_resp *)xfer->tx_message.buf;
...
fail:
	dev_dbg(info->dev, "RM_UDMA TX: chn %u ret:%u\n", *index, ret);
	return ret;
}
					  ...
					  u32 index)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_UDMAP_TX_FREE,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
		...
		u32 *rng_flow_start_index, u32 *rng_flow_cnt)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_UDMAP_RX_ALLOC,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
	...
	resp = (struct ti_sci_msg_udmap_rx_ch_alloc_resp *)xfer->tx_message.buf;
...
fail:
	dev_dbg(info->dev, "RM_UDMA RX: chn %u def_flow_index rng %u_flow_start_index %u rng_flow_cnt %u ret:%u\n",
		*index, *def_flow_index, *rng_flow_start_index,
		*rng_flow_cnt, ret);
	return ret;
}
					  ...
					  u32 index)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_UDMAP_RX_FREE,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
			...
			const struct ti_sci_rm_udmap_rx_flow_cfg *params)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_UDMAP_FLOW_CFG,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
			...
			const struct ti_sci_rm_udmap_rx_flow_opt_cfg *params)
{
	...
	xfer = ti_sci_setup_one_xfer(info, TI_SCI_MSG_RM_UDMAP_OPT_FLOW_CFG,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (u32 *)&req, sizeof(req), sizeof(*resp));
	if (IS_ERR(xfer)) {
		ret = PTR_ERR(xfer);
