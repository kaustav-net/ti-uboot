...
{
	struct udevice *dev = ring->parent->dev;
	...
	if (ring->ring_mem_virt)
		print_buffer((ulong)ring->ring_mem_virt,
			     ring->ring_mem_virt, 4,
			     (ring->size * (4 << ring->elm_size)) / 4, 0);
}
#endif
...
static int k3_nav_ringacc_probe_dt(struct k3_nav_ringacc *ringacc)
{
	...
	ret = dev_read_u32_array(dev, "ti,gp-rings", &ranges[0], 2);
	if (ret) {
		ranges[0] = 0;
